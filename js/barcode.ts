import { YAPI, YErrorMsg, YModule } from './yocto-api-esm/yocto_api_html.js';
...
function decodeMH10code(hexdata: string): MH10Dict
{
    let endpos: number = (hexdata.length >>> 1);
    let codes: number[] = [];
    ...
    do {
        let pos: number = data.indexOf('\u001d', prevpos);
        if(pos < 0) pos = endpos;
        let field: string = data.slice(prevpos, pos).trim().replace(/\u001e/g,'<RS>');
        if(field.slice(0,4) == 'GWCR') {
            ...
            console.log(matches);
            let fmt: number = parseInt(matches[1]);
            let datecode: string = matches[2];
            let datefmt: string = '';
            let date = new Date();
            ...
            let monthDay: number = 1;
            fields['datecode'] = datecode;
            switch(fmt) {
                case 0: datefmt = 'ymd'; break;
                case 1: datefmt = 'dmy'; break;
                case 2: datefmt = 'mdy'; break;
                case 3: datefmt = 'iz'; break;
                case 4: datefmt = 'yz'; break;
                case 5: datefmt = 'ymd'; break;
                case 6: datefmt = 'Ymd'; break;
                case 7: datefmt = 'my'; break;
                case 10: datefmt = 'yw'; break;
                case 11: datefmt = 'Yw'; break;
                case 12: datefmt = 'Ymd'; break;
                case 13: datefmt = 'yw'; break;
                case 16: datefmt = 'Ymd'; break;
                case 17: datefmt = 'dmY'; break;
            }
...
async function barcodeHandler(port: YSerialPort, change: string): Promise<void>
{
    let messages: string[] = await port.readMessages('',1);
    await port.reset();
    if(messages.length == 0 || !messages[0]) return;
    let form: HTMLFormElement = document.getElementsByName('panelform').item(0) as HTMLFormElement;
...
async function enableDeviceHandlers(): Promise<void>
{
